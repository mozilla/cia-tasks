...
from mo_dots import Data, coalesce, listwrap, concat_field, set_default, to_data
...
class Schedulers:
    def __init__(self, config):
        ...
        self.destination = bigquery.Dataset(config.destination).get_or_create_table(config.destination)
    ...
    def set_state(self):
        adr_configuration.config.cache.put(
            CACHY_STATE, value2json(self.done), minutes=CACHY_RETENTION
        )
    ...
    def get_task_details(self, push):
        ...
        task_timings = http.get_json(
            ...
            json={
                ...
                "where": [
                    {"eq": {"repo.push.id": push.id}},
                    {"eq": {"repo.branch.name": push.branch}},
                ],
                "format": "list",
                "limit": 100000,
            },
    ...
    def process_chunk_of_pushes(self, start, end, branch, please_stop):
        """
                ...
                with Timer("get scheduler tasks for push {{push}}", {"push": push.id}):
                    ...
                    for s in schedulers:
                        try:
                            scheduler.append({
                                "name": s,
                                "tasks": jx.sort(push.get_shadow_scheduler_tasks(s)),
                            })
                        except Exception:
                            pass
                try:
    ...
    def process(self, please_stop):
        ...
        try:
            for start, end, branch in self.todo:
                if please_stop:
                    break
                self.process_chunk_of_pushes(start, end, branch, please_stop)
        except Exception as e:
...
def main():
    try:
        ...
        def update(self, config):
            ...
            self._config["sources"] = sorted(map(
                os.path.expanduser, set(self._config["sources"])
            ))
